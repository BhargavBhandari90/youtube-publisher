    ...
    try {
      ...
      if (res.ok) {
        ...
        const matchedCategory = youtubeCategories.find(
          (cat) =>
            cat.snippet.title.toLowerCase() === data.category.toLowerCase()
        );
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      >
        ...
        {videoPreview && (
          <div className="flex justify-center items-center mt-4">
            <video
              src={videoPreview}
              controls
              className="w-full max-w-md rounded-md border mt-4"
            />
          </div>
        )}
        ...
        >
          {generating ? (
            <>
              <IconReload size={40} stroke={2} className="animate-spin" />{" "}
              Generating Metadata...
            </>
          ) : (
            <>
      ...
      {response ? (
        <div className="space-y-4 bg-gray-900 p-6 rounded-xl shadow-lg">
          ...
          <div className="bg-gray-800 text-white rounded-xl shadow-lg p-6 space-y-4 max-w-2xl mx-auto mt-6">
            <div>
              <h2 className="text-lg font-semibold">📌 Title</h2>
              <p className="text-gray-400">{response.title}</p>
            </div>
      ...
      {uploadedToYT && (
        <>
          <SuccessPopup show={uploadedToYT} onClose={() => setUploadedToYT(false)} />
